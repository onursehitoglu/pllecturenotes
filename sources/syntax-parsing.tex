...
\item Left recursion in grammar is a problem. Elimination of left recursion
	needed.
\item \structure{Deterministic parsing:} Look at input symbols to choose the 
	next rule to apply.
...
int accept(Symbol s) { if (sym == s) { next(); return 1; } 
	return 0; // consume and match a nonterminal symbol
}
void factor(void) { // <factor> -> <ident> | <number> | ( <expr> )
    if (accept(ident)) ;
    else if (accept(number)) ;
    else if (accept(lparen)) { expression(); expect(rparen);}
    else { error("factor: syntax error at ",currsym); next(); }
}
void term(void) { // <term> -> <factor> | <factor> * <factor>
    factor();
    while (accept(times) || accept(slash))
        factor();
}
void expression(void) { // <expression> -> <term> | <term> + <term>
    term();
...
\begin{itemize}
\item Start from input sentence and merge parts of sentential forms matching RHS of a rule into LHS at each step. Try to reach the starting non-terminal.
	\\ {\small
	\[ \begin{array}{l}
	~~ a = a + b * a  ~~ \mapsto
...
\end{array}
	\]}
\item Simplest form gives the rightmost derivation of a grammar (in reverse) processing input
	from left to right.
...
\begin{frame}
\begin{itemize}
\item Shift reduce parser needs to try all non-deterministic shift combinations to get all parses.
\item For deterministic parsing, states based on input lookahead or
	precedence are required.
