...
parse(State, [H|R]) :- trans(State, New, H), parse(New,R).
parse(State, Inp) :- empty(State, New), parse(New,Inp).
...
Use combination for the symmetric operators, \lstinline!taketwo! gets
 \lstinline!a, b!  once, not \lstinline!b, a!.
\begin{beamercolorbox}{oexample}
\begin{lstlisting}
taketwo(A, B, [A|R] , Rem ) :- select(B, R, Rem). % pick B for first A
taketwo(A, B, [H|R], [H|Rem]) :- taketwo(A, B, R, Rem). % pick 2 from rest

% if top element evaluates to the target number, it is the result
findit([A|_],V,A) :- V is A. 
% asymmetric ops.
findit(L,V,T) :- select(A,L,AR), select(B,AR,Rest),
        member(O,[-,//]), check(O,A,B), E =.. [O,A,B],
...
findit(L,V,T) :- taketwo(A,B,L,Rest),
        member(O,[+,*]),  E =.. [O,A,B],
        findit([E|Rest],V,T).
% make sure subtraction and division are correct
check(-,A,B) :- A>B.
check(//,A,B) :- 0 is A mod B.
\end{lstlisting}
\end{beamercolorbox}
