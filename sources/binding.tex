...
\begin{itemize}
 ...
 \item \structure{Fortran} and \structure{C} functions have flat block structure.\\
{\tiny \tt
...
\ \begin{tabular}{|>{\columncolor{blue!5!white}}p{.8\linewidth}|} \hline
   ...
   ....\\ \hline
\end{tabular}}
\item \structure{C} block commands can be nested.
\item \structure{GCC} extensions (\structure{Not in any C standard!}) to C allow nested function definitions.
\end{itemize}
\end{frame}
...
in func x + let other x = x - 1
            ...
            in func x
\end{lstlisting}
\item Static binding (default) binds outer \lstinline{other} for both \lstinline{func} calls.
	In dynamic binding (hypothetical), first call body binds outer \lstinline{other} with \lstinline{y=10}, 
	second call body binds inner \lstinline{other} with \lstinline{y=2},
\end{beamercolorbox}
\end{itemize}
\end{frame}
...
\begin{itemize}
 \item \texttt{Start; D$_1$ and D$_2$ and ...  and D$_n$ ; End}
...
int x=3, i=2;
x += i;
for (i = 0; x > i ; i++) {  /* i are bound to the outer i */
    int i = 0; /* this i hides the i above */
    ...
    i++;       /* this is bound to local i */
}
/* inner i is not accessible */
\end{lstlisting}
\end{beamercolorbox}
\end{itemize}
...
\begin{beamercolorbox}{hexample}
\begin{lstlisting}[language={Haskell},escapechar=\#,basicstyle=\tiny\ttfamily]
gcd a b | (a < 0) || (b < 0) = error "negative"
        | a < b = localgcd a b
        | otherwise = localgcd b a  
    where 
       localgcd a b | a == 0 = b
                    | otherwise = localgcd (b `mod` a) a
\end{lstlisting}
\end{beamercolorbox}
\item A hypothetical syntax (\lstinline{let} is only for expressions):
\begin{beamercolorbox}{hexample}
\begin{lstlisting}[language={Haskell},escapechar=\#,basicstyle=\tiny\ttfamily]
let -- following declarations are local only
   data Hidden k v = ... -- actual data structure is hidden
   inserttree k v ... = ... -- internal functions are hidden
   traversetree ... = ...
 in -- only the following declarations are available outside
   data DS k v  = T (Hidden k v) deriving Show
   emptyds = T HEmpty
   insertds k v (T hidden) = T $ inserttree k v hidden
   traverseds (T hidden) = traversetree hidden
\end{lstlisting}
\end{beamercolorbox}
\end{itemize}
}
